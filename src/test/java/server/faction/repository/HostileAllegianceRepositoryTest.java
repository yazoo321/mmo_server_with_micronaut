...
public class HostileAllegianceRepositoryTest {
    ...
    public void testFindHostilitiesByAllegiance() {
        List<HostileAllegiance> expected = List.of(
                new HostileAllegiance(GUILD_A, GUILD_B, hostilityLevel),
                new HostileAllegiance(GUILD_A, GUILD_C, hostilityLevel)
        );

        hostileAllegianceRepository.upsertHostility(GUILD_A, GUILD_B, hostilityLevel).blockingSubscribe();
        hostileAllegianceRepository.upsertHostility(GUILD_A, GUILD_C, hostilityLevel).blockingSubscribe();


        // Insert hostility record for later retrieval
        List<HostileAllegiance> actual = hostileAllegianceRepository.findHostilitiesByAllegiance(List.of(GUILD_A))
                .blockingGet();
