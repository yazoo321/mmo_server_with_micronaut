...
public class SynchroniseDroppedItemsService {
        ...
        itemService.getItemsInMapAsync(new Location(motion))
                ...
                .doOnSuccess(droppedItems -> {
                    Map<String, DroppedItem> droppedItemsMap =
                            droppedItems.stream().collect(Collectors.toMap((i) ->
                                    i.getItemInstance().getItemInstanceId(), Function.identity()));
                    ...
                    session.asMap().put(SessionParams.DROPPED_ITEMS.getType(), currentItems);
                })
                .subscribe();
    }

    private Set<String> getTrackedItems(WebSocketSession session) {
        return (Set<String>) session.asMap().getOrDefault(SessionParams.DROPPED_ITEMS.getType(), List.of());
    }

    private Set<String> getLostItemIds(Set<String> currentItems, Set<String> trackedItems) {
        return trackedItems.stream().filter(i -> !currentItems.contains(i)).collect(Collectors.toSet());
    }

    private Set<String> getNewItemIds(Set<String> currentItems, Set<String> trackedItems) {
        return currentItems.stream().filter(i -> !trackedItems.contains(i)).collect(Collectors.toSet());
    }

    private void handleNewItems(Set<String> currentItems, Set<String> trackedItems,
                                Map<String, DroppedItem> droppedItemsMap, WebSocketSession session) {
    ...
    private void handleLostItems(Set<String> currentItems, Set<String> trackedItems,
                                 Map<String, DroppedItem> droppedItemsMap, WebSocketSession session) {
