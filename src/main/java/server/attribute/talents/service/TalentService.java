...
public class TalentService {
    ...
    public void fetchAllTalentsForTree(WebSocketSession session, String treeName) {
        ...
        TalentData talentData = TalentData.builder()
                .talentTree(tree)
    ...
    public void fetchActorLearnedTalents(String actorId, WebSocketSession session) {
        ...
        talentRepository.getActorTalents(actorId)
                .doOnSuccess(actorTalents -> {
                    SocketResponse response = new SocketResponse();
                    TalentData talentData = TalentData.builder()
                            ...
                            .build();
                    response.setTalentData(talentData);
                    response.setMessageType(SocketResponseType.AVAILABLE_ACTOR_TALENTS.getType());
    ...
    public Map<String, Integer> getPointsPerTree(ActorTalents actorTalents) {
        ...
        actorTalents.getLearnedTalents().forEach((key, value) ->
                pointsPerTree.merge(key, value, Integer::sum));
    ...
    private Single<Map<String, Integer>> fetchAvailableTalents(String actorId) {
        ...
        return Single.zip(actorStatsSingle, actorTalentsSingle, (stats, actorTalents) -> {
            Map<String, Integer> pointsPerTree = getPointsPerTree(actorTalents);

            talents.forEach((talentName, talent) -> {
                int canLevelTo = talent.isAvailableForCharacter(stats, actorTalents.getLearnedTalents(), pointsPerTree);
                if (canLevelTo > 0) {
    ...
    public void learnTalent(WebSocketSession session, String talentName, int level) {
        ...
        fetchAvailableTalents(actorId)
                .doOnSuccess(availableToLevel -> {
                    if (!availableToLevel.containsKey(talentName)) {
                        log.warn("Actor: {}, tried to learn a talent: {} they haven't met dependencies for",
                                actorId, talentName);
                    ...
                    if (level != expectedToLevel) {
                        log.warn("Actor {} tried to learn talent: {} with level: {}, but it's currently level: {}",
                                actorId, talentName, level, expectedToLevel);
                    ...
                    talentRepository.getActorTalents(actorId)
                            .doOnSuccess(actorTalents -> {
                                ...
                                talentRepository.insertActorTalents(actorId, actorTalents)
                                        .doOnSuccess(updated -> {
                                            SocketResponse response = new SocketResponse();
                                            TalentData talentData = TalentData.builder()
                            ...
                            .doOnError(err -> log.error("Error processing in learning talent, {}", err.getMessage()))
                            .subscribe();
                })
                .subscribe();
    }
