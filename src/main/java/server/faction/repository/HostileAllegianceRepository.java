...
//        prepareCollections();
    ...
    public Single<List<HostileAllegiance>> findHostilitiesByAllegiance(List<String> allegianceName) {
        return Flowable.fromPublisher(hostileAllegianceCollection.find(
                or(in("hostileTo", allegianceName),
                        in("allegianceName", allegianceName))))
                .toList();
    }

    @CacheInvalidate(value = HOSTILE_ALLEGIANCE_CACHE, parameters = "allegianceName", async = true)
    public Single<HostileAllegiance> upsertHostility(String allegianceName, String hostileTo, int hostilityLevel) {
        ReplaceOptions options = new ReplaceOptions().upsert(true);
        Bson filter = and(eq("allegianceName", allegianceName), eq("hostileTo", hostileTo));

        HostileAllegiance hostileAllegiance = new HostileAllegiance(allegianceName, hostileTo, hostilityLevel);
        return Single.fromPublisher(hostileAllegianceCollection.replaceOne(
                        filter, hostileAllegiance, options))
                .doOnError(err -> log.error(err.getMessage()))
                .map(res -> hostileAllegiance);
    }

    public Single<DeleteResult> delete(String allegianceName, String hostileTo) {
        return Single.fromPublisher(hostileAllegianceCollection.deleteOne(
                and(eq("allegianceName", allegianceName),
                        eq("hostileTo", hostileTo))
        ));
    }

    @PostConstruct
    private void prepareCollections() {
        this.hostileAllegianceCollection = mongoClient
